...
| Check | Command |
|-------|---------|
| App is deployed (Phase 4) | `kubectl get pods -n cloudkitchen` — all Running |
| Traefik LB hostname known | `kubectl get svc traefik -n ingress` — `EXTERNAL-IP` populated |
...
```bash
LB_DNS=$(kubectl get svc traefik -n ingress \
