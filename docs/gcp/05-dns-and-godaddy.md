...
> Before: `http://<LB_IP>/`
> After:  `http://harishraodevops.in/`  and  `http://harishraodevops.in/argocd/`
...
| A domain you control                              | We use `harishraodevops.in` (registered at GoDaddy)           |
...
> 💡 We used `Name = @` so the bare `harishraodevops.in` resolves. With
> `Name = cloudkitchen` the URL becomes `cloudkitchen.harishraodevops.in`.
...
```bash
dig +short @8.8.8.8 harishraodevops.in
```
...
```bash
nslookup harishraodevops.in 8.8.8.8
```
Expected:
```
Name:    harishraodevops.in
...
ingress:
  ...
  hosts:                        # 👈 a list (one item per accepted hostname)
    - harishraodevops.in         # 👉 YOUR hostname goes here
...
```bash
HOSTNAME='harishraodevops.in'                                                  # 👈 set to YOUR domain
...
```
    - match: (Host(`harishraodevops.in`) || Host(`35.224.38.103`)) && PathRegexp(`^/api/restaurants/...`)
    - match: (Host(`harishraodevops.in`) || Host(`35.224.38.103`)) && PathPrefix(`/api/menu`)
    - match: (Host(`harishraodevops.in`) || Host(`35.224.38.103`)) && PathPrefix(`/api/auth`)
    - match: (Host(`harishraodevops.in`) || Host(`35.224.38.103`)) && PathPrefix(`/api/users`)
    - match: (Host(`harishraodevops.in`) || Host(`35.224.38.103`)) && PathPrefix(`/api/restaurants`)
```
...
git commit -m "phase 5: accept hostname (harishraodevops.in) on the IngressRoute"
...
Open in a browser:
- **App:** http://harishraodevops.in/
- **ArgoCD:** http://harishraodevops.in/argocd/  (trailing slash required)
...
| `nslookup` and `dig` disagree | They're querying different DNS servers | Force both to Google: `nslookup harishraodevops.in 8.8.8.8` and `dig +short @8.8.8.8 harishraodevops.in`. |
