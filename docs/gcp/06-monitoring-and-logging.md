...
> After:  http://harishraodevops.in/grafana/      — dashboards
>         http://harishraodevops.in/prometheus/   — Prometheus UI
>         http://harishraodevops.in/alertmanager/ — Alertmanager UI
...
| Need                                              | How to check                                                            |
| ------------------------------------------------- | ----------------------------------------------------------------------- |
| Phase 5 done (DNS works, app reachable at hostname) | `curl -sI http://harishraodevops.in/`  → HTTP 200                       |
...
```yaml
        ...
        grafana:
          ...
          grafana.ini:
            server:
              ...
              root_url: "http://harishraodevops.in/grafana/"
        ...
        prometheus:
          prometheusSpec:
            ...
            externalUrl: http://harishraodevops.in/prometheus    # 👈
        ...
        alertmanager:
          ...
          alertmanagerSpec:
            ...
            externalUrl: http://harishraodevops.in/alertmanager   # 👈
            resources:
...
> ⚠️ Replace the two host values (`harishraodevops.in` and `35.224.38.103`)
...
spec:
  entryPoints:
    - web
  routes:
    - match: (Host(`harishraodevops.in`) || Host(`35.224.38.103`)) && PathPrefix(`/grafana`)
      kind: Rule
      services:
...
spec:
  entryPoints:
    - web
  routes:
    - match: (Host(`harishraodevops.in`) || Host(`35.224.38.103`)) && PathPrefix(`/prometheus`)
      kind: Rule
      services:
...
spec:
  entryPoints:
    - web
  routes:
    - match: (Host(`harishraodevops.in`) || Host(`35.224.38.103`)) && PathPrefix(`/alertmanager`)
      kind: Rule
      services:
...
```bash
curl -sIL -o /dev/null -w "/grafana/      -> HTTP %{http_code}\n" "http://harishraodevops.in/grafana/"
curl -sIL -o /dev/null -w "/prometheus/   -> HTTP %{http_code}\n" "http://harishraodevops.in/prometheus/"
curl -sL "http://harishraodevops.in/alertmanager/-/ready"   # prints "OK"
```

Open in browser:
- **Grafana:** http://harishraodevops.in/grafana/  — login `admin / prom-operator`
- **Prometheus:** http://harishraodevops.in/prometheus/
- **Alertmanager:** http://harishraodevops.in/alertmanager/
...
```bash
curl -s "http://harishraodevops.in/prometheus/api/v1/query?query=up%7Bnamespace%3D%22cloudkitchen%22%7D" \
...
```bash
curl -s "http://harishraodevops.in/prometheus/api/v1/query?query=sum%20by(service)%20(http_requests_total%7Bnamespace%3D%22cloudkitchen%22%7D)" \
  | python3 -m json.tool
```
...
1. Open **http://harishraodevops.in/grafana/** and log in.
...
curl -s -u "admin:${GRAFANA_PWD}" \
  "http://harishraodevops.in/grafana/api/search?type=dash-db&query=CloudKitchen" \
  | python3 -m json.tool
```
...
| Service              | URL                                                                                  |
| -------------------- | ------------------------------------------------------------------------------------ |
| auth-service         | http://harishraodevops.in/grafana/d/ck-auth-service/cloudkitchen-auth-service         |
| user-service         | http://harishraodevops.in/grafana/d/ck-user-service/cloudkitchen-user-service         |
| restaurant-service   | http://harishraodevops.in/grafana/d/ck-restaurant-service/cloudkitchen-restaurant-service |
| menu-service         | http://harishraodevops.in/grafana/d/ck-menu-service/cloudkitchen-menu-service         |
| order-service        | http://harishraodevops.in/grafana/d/ck-order-service/cloudkitchen-order-service       |
| payment-service      | http://harishraodevops.in/grafana/d/ck-payment-service/cloudkitchen-payment-service   |
| delivery-service     | http://harishraodevops.in/grafana/d/ck-delivery-service/cloudkitchen-delivery-service |
| notification-service | http://harishraodevops.in/grafana/d/ck-notification-service/cloudkitchen-notification-service |
...
Open the **Prometheus UI** at **http://harishraodevops.in/prometheus/**,
...
> `curl -X POST http://harishraodevops.in/api/auth/login -d 'bogus'`.
...
Open the Alertmanager UI at **http://harishraodevops.in/alertmanager/** to see:
