...
```
https://harishraodevops.in                — the app UI
https://harishraodevops.in/argocd         — ArgoCD UI
https://harishraodevops.in/grafana        — Grafana
https://harishraodevops.in/prometheus     — Prometheus
https://harishraodevops.in/alertmanager   — Alertmanager
```
...
| Check | How |
|---|---|
| Phase 5 done (your domain resolves to the LB IP) | `dig +short harishraodevops.in` → returns your LB IP (e.g. `136.112.45.103`) |
...
spec:
  ...
  sourceRepos:
    - https://github.com/harishrao-devops/cloud-kitchen-app.git 
...
> 📧 Email is hard-coded to `harishrai1195@gmail.com`. Let's Encrypt only
...
spec:
  acme:
    ...
    email: harishrao1195@gmail.com         # 👈 used for expiry notices
    privateKeySecretRef:
...
# Try it yourself:
curl -v http://harishraodevops.in/.well-known/acme-challenge/test
...
spec:
  ...
  routes:
    ...
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && PathRegexp(`^/api/restaurants/[^/]+/(menu|categories|items)`)
      kind: Rule
      priority: 200
      services:
        - {name: menu-service, port: 8080}
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && PathPrefix(`/api/menu`)
      kind: Rule
      priority: 200
      services:
        - {name: menu-service, port: 8080}
    # --- one rule per service ---
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && PathPrefix(`/api/auth`)
      kind: Rule
      priority: 100
      services:
        - {name: auth-service, port: 8080}
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && PathPrefix(`/api/users`)
      kind: Rule
      priority: 100
      services:
        - {name: user-service, port: 8080}
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && PathPrefix(`/api/restaurants`)
      kind: Rule
      priority: 100
      services:
        - {name: restaurant-service, port: 8080}
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && (PathPrefix(`/api/cart`) || PathPrefix(`/api/orders`))
      kind: Rule
      priority: 100
      services:
        - {name: order-service, port: 8080}
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && PathPrefix(`/api/payments`)
      kind: Rule
      priority: 100
      services:
        - {name: payment-service, port: 8080}
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && PathPrefix(`/api/deliveries`)
      kind: Rule
      priority: 100
      services:
        - {name: delivery-service, port: 8080}
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && PathPrefix(`/api/notifications`)
      kind: Rule
      priority: 100
      services:
        - {name: notification-service, port: 8080}
    # --- frontend catch-all (lowest priority) ---
    - match: (Host(`harishraodevops.in`) || Host(`136.112.45.103`)) && PathPrefix(`/`)
      kind: Rule
      priority: 1
      services:
...
```bash
curl -sI "https://harishraodevops.in/" | head -1
# Expect: HTTP/2 200

curl -s "https://harishraodevops.in/api/restaurants" | head -c 200 ; echo
# Expect: JSON array of restaurants

# All 10 microservice routes still work — try a couple more:
curl -s -o /dev/null -w "HTTP %{http_code}\n" "https://harishraodevops.in/api/auth/health"
curl -s -o /dev/null -w "HTTP %{http_code}\n" "https://harishraodevops.in/api/menu"
```
...
Right now those four UIs are reachable at `http://harishraodevops.in/argocd/`,
...
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(`harishraodevops.in`) && PathPrefix(`/grafana`)
      kind: Rule
      services:
...
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(`harishraodevops.in`) && PathPrefix(`/prometheus`)
      kind: Rule
      services:
...
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(`harishraodevops.in`) && PathPrefix(`/alertmanager`)
      kind: Rule
      services:
...
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(`harishraodevops.in`) && PathPrefix(`/argocd`)
      kind: Rule
      services:
        - name: argocd-server
          port: 80
...
- https://harishraodevops.in/                  → React UI
- https://harishraodevops.in/argocd/           → ArgoCD UI (admin / your bootstrap password)
- https://harishraodevops.in/grafana/          → Grafana (admin / prom-operator)
- https://harishraodevops.in/prometheus/       → Prometheus
- https://harishraodevops.in/alertmanager/     → Alertmanager
...
If you `curl http://harishraodevops.in/` right now you'll see:
...
for path in / argocd/ grafana/login prometheus/-/ready alertmanager/-/ready; do
  ...
  curl -sI "http://harishraodevops.in/${path#/}" \
    | awk '/^HTTP|^[Ll]ocation:/ {printf "%s ", $0}' ; echo
done
# Expect for each:  HTTP/1.1 308 Permanent Redirect  Location: https://harishraodevops.in/...

# And follow the redirect — should land on 200 over HTTPS
for path in / argocd/ grafana/login prometheus/-/ready alertmanager/-/ready; do
  printf "%-30s -> %s\n" "http://...${path}" \
    "$(curl -sIL -o /dev/null -w '%{http_code} %{url_effective}' "http://harishraodevops.in/${path#/}")"
done
# Expect:  200 https://harishraodevops.in/...  for every path
```
...
|---|---|---|
| `Certificate` stuck `READY=False` for >5 min | Let's Encrypt HTTP-01 can't reach `http://harishraodevops.in/.well-known/acme-challenge/…` | Confirm port 80 is open: `curl http://harishraodevops.in/.well-known/acme-challenge/test` should return 404 (not "connection refused"). `kubectl get challenges -A` shows the exact URL Let's Encrypt is probing — curl it from your laptop. |
| Browser shows "NET::ERR_CERT_AUTHORITY_INVALID" or warns | Either the certificate isn't ready yet OR you're hitting Traefik via an IP without the matching `Host:` header (Traefik then serves a self-signed default cert) | Confirm `kubectl -n cloudkitchen get certificate cloudkitchen-tls` shows `READY=True`, and curl with the hostname (`curl https://harishraodevops.in/`), not the IP. |
...
```
https://harishraodevops.in                📱 the app
https://harishraodevops.in/argocd         🚀 GitOps controller
https://harishraodevops.in/grafana        📊 dashboards
https://harishraodevops.in/prometheus     📈 metrics
https://harishraodevops.in/alertmanager   🚨 alerts
```
...
🏁 The full DevOps lifecycle in seven phases on GKE:
